...
import { formatManilaDate } from '@/lib/reservations/reservation-monitoring';
...
function getExpirationExplanation(reservation: Reservation) {
  if (reservation.expirationReason !== 'pending_approval_deadline') {
    return 'This reservation is no longer active.';
  }

  const expiredDate = formatManilaDate(reservation.expiredAt);
  if (!expiredDate) {
    return 'This reservation request expired because it was still pending when the reservation date arrived.';
  }

  return `This reservation request expired on ${expiredDate} because it was still pending when the reservation date arrived.`;
}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isExpired && (
                        <p
                          ...
                          title={getExpirationExplanation(reservation)}
                        >
                          Expired{formatManilaDate(reservation.expiredAt) ? ` on ${formatManilaDate(reservation.expiredAt)}` : ''}
                        </p>
                      )}
                      {isExpired && reservation.expirationMessage?.message && (
                        <div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
                          <p className="text-xs font-bold text-blue-900">Admin message</p>
                          <p className="mt-1 whitespace-pre-wrap text-sm text-blue-950">
                            {reservation.expirationMessage.message}
                          </p>
                        </div>
                      )}
                    </div>
